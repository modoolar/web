...
odoo.define('web_diagram_position.DiagramRenderer', function (require) {
    ...
    const DiagramRenderer = require('web_diagram.DiagramRenderer');
    const graphLib = require('web_diagram_position.graph');
    const MyCuteGraph = graphLib.MyCuteGraph;
    const MyCuteNode = graphLib.MyCuteNode;
    const MyCuteEdge = graphLib.MyCuteEdge;
    DiagramRenderer.include({

        track_node_position_changes: function () {
            const model = this.getParent().model;
            return typeof model.nodes.attrs.xpos !== "undefined" && typeof model.nodes.attrs.ypos !== "undefined";
        },

        on_node_drag_end: function (e) {
            const self = this;
            const node = e.target.cute_node;
            if (typeof e.target.cute_node === "undefined" || self.node_updating) {
                return;
            }
        ...
        create_cute_edge: function (graph, edge, nodes) {
            return new MyCuteEdge(
                ...
                nodes[edge.s_id],
                //WORKAROUND
                nodes[edge.d_id] || nodes[edge.s_id]
            );
        },
        ...
        make_cute_node: function (graph, node, style) {
            const n = this.create_cute_node(graph, node, style);
            n.id = node.id;
        ...
        make_tracked_cute_node: function (graph, node, style) {
            const n = this.make_cute_node(graph, node, style);

            const self = this;

            const drag_end = function (e) {
        ...
        _render: function () {
            ...
            const r = new Raphael($div[0], '100%', '100%');
            const graph = self.create_cute_graph(r, style, this.$diagram_container[0]);
            this.graph = graph;

            const make_node = this.track_node_position_changes() ? this.make_tracked_cute_node.bind(this) : this.make_cute_node.bind(this);
            ...
            _.each(edges, function (edge) {
                const e = self.create_cute_edge(graph, edge, self.id_to_node);
                e.id = edge.id;
            ...
            MyCuteEdge.creation_callback = function () {
                return {label: ''};
            };
