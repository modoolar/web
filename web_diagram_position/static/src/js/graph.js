...
odoo.define("web_diagram_position.graph", function () {

    // this serves as the end of an edge when creating a link
    function MyEdgeEnd(pos_x, pos_y) {
        this.x = pos_x;
        this.y = pos_y;
    ...
    // A close button,
    // if entity_type === "node":
    //      MyGraphNode.destruction_callback(entity) is called where entity is a node.
    //      If it returns true the node and all connected edges are destroyed.
    // if entity_type === "edge":
    //      MyGraphEdge.destruction_callback(entity) is called where entity is an edge
    ...
    function MyCloseButton(graph, entity, entity_type, pos_x, pos_y) {
        var self = this;
        var visible = false;
        var close_button_radius = graph.style.close_button_radius || 8;
        var close_circle = graph.r.circle(entity.get_pos().x + pos_x, entity.get_pos().y + pos_y, close_button_radius);
        ...
        function click_action() {
            ...
            if (entity_type === "node") {
                $.when(MyGraphNode.destruction_callback(entity)).done(function () {
                    entity.remove();
                });
            } else if (entity_type === "edge") {
                $.when(MyGraphEdge.destruction_callback(entity)).done(function () {
                    entity.remove();
                });
            }
        }
    ...
    function MyConnector(graph, node, pos_x, pos_y) {
        var visible = false;
        ...
        this.update_pos = function () {
            conn_circle.attr({
                'cx': node.get_pos().x + pos_x,
                'cy': node.get_pos().y + pos_y
            });
        };
        ...
        function hover_in() {
            ...
            if (graph.creating_edge) {
                ...
                conn_circle.animate({
                    'fill': graph.style.connector_active_color,
        ...
        function hover_out() {
            ...
            conn_circle.animate({
                'r': graph.style.connector_radius,
                'fill': graph.style.node_outline_color,
                'stroke': 'none'
            }, 400, 'linear');
        ...
        var drag_down = function () {
            ...
            self.edge_start = new MyEdgeEnd(self.ox, self.oy);
            self.edge_end = new MyEdgeEnd(self.ox, self.oy);
            self.edge_tmp = new MyGraphEdge(graph, '', self.edge_start, self.edge_end, true);
        ...
        var drag_up = function () {
            ...
            if (graph.target_node) {
                var edge_prop = MyGraphEdge.creation_callback(node, graph.target_node);
                if (edge_prop) {
                    var new_edge = new MyGraphEdge(graph, edge_prop.label, node, graph.target_node);
                    MyGraphEdge.new_edge_callback(new_edge);
                }
            }
        };
    ...
    function MyGraph(r, style, viewport, options) {
        var self = this;
        ...
        self.readonly = options && options.readonly !== undefined ? options.readonly : false;
        ...
        var background = r.rect(0, 0, '100%', '100%').attr({
            'fill': 'white',
            'stroke': 'none',
            'opacity': 0,
            'cursor': 'move'
        ...
        // MyGraph translation when background is dragged
    ...
    function MyGraphNode(graph, pos_x, pos_y, label, type, color) {
        ...
        node_label.attr({
            'fill': graph.style.node_label_color,
        ...
        var set_selected = function () {
            if (!selected) {
                selected = true;
                node_fig.attr({
                    'stroke': graph.style.node_selected_color,
                    'stroke-width': graph.style.node_selected_width
                });
                if (!self.close_button) {
                    self.close_button = new MyCloseButton(graph, self, "node", sx / 2, -sy / 2);
        ...
        function double_click() {
            MyGraphNode.double_click_callback(self);
        }
        ...
        this.connectors.push(new MyConnector(graph, this, -sx / 2, 0));
        this.connectors.push(new MyConnector(graph, this, sx / 2, 0));
        this.connectors.push(new MyConnector(graph, this, 0, -sy / 2));
        this.connectors.push(new MyConnector(graph, this, 0, sy / 2));

        if (!graph.is_readonly()) {
            this.close_button = new MyCloseButton(graph, this, "node", sx / 2, -sy / 2);
        }
    }

    MyGraphNode.double_click_callback = function (node) {
    ...
    MyGraphNode.destruction_callback = function (node) {
        return true;
    };
    ...
    function MyGraphEdge(graph, label, start, end, tmp) {
        var self = this;
        var r = graph.r;
        ...
        update_curve();
        var edge = r.path(edge_path).attr({
            'stroke': graph.style.edge_color,
            'stroke-width': graph.style.edge_width,
            'arrow-end': 'block-wide-long',
            'cursor': 'pointer'
        }).insertBefore(graph.get_node_list()[0].get_fig());
        ...
        this.set_selected = function () {
            if (!selected) {
                ...
                if (!self.close_button) {
                    self.close_button = new MyCloseButton(graph, self, "edge", 0, 30);
                    self.close_button.show();
                }
            }
        };
        ...
        this.set_label = function (text) {
            edge_label.attr({'text': text});
        };
        ...
        function double_click_action() {
            MyGraphEdge.double_click_callback(self);
        }
    ...
    MyGraphEdge.double_click_callback = function (edge) {
    ...
    MyGraphEdge.creation_callback = function (start, end) {
        var edge_prop = {};
    ...
    MyGraph.wordwrap = wordwrap;

    return {
        MyCuteGraph: MyGraph,
        MyCuteNode: MyGraphNode,
        MyCuteEdge: MyGraphEdge,
    };
});
